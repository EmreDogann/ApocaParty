    ...
    public class OrthogonalUnitVector3Drawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (chosenInvert)
                {
                    SetVectorComponent(property, _enumNames[_axis], -1.0f);
                }
                else
                {
                    SetVectorComponent(property, _enumNames[_axis], 1.0f);
        ...
        private void SetVectorComponent(SerializedProperty property, string componentName, float invert)
        {
            if (componentName == "X")
            {
                property.vector3Value = Vector3.right * invert;
            }
            else if (componentName == "Y")
            {
                property.vector3Value = Vector3.up * invert;
            }
            else
            {
                property.vector3Value = Vector3.forward * invert;
            }
        }
    }
}